...
class FileLockResolver implements SequenceResolver
{
    ...
    protected function getSequence(string $filePath, int $currentTime): int
    {
        ...
        $this->updateContents($contents = $this->incrementSequenceWithSpecifyTime($this->cleanOldSequences($contents), $currentTime), $f);
    ...
    {
        return ftruncate($f, 0) && rewind($f) && (fwrite($f, serialize($contents)) !== false);
    }
